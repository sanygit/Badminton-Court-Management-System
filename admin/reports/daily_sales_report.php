        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
                    ...
                    $total = 0;
					$i = 1;
                    $qry = $conn->query("SELECT si.*, s.client_name, p.name as `product`, s.date_created from `sales_transaction_items` si inner join sales_transaction s on si.sales_transaction_id = s.id inner join product_list p on si.product_id = p.id where date(s.date_created) = '{$date}' order by unix_timestamp(s.date_created) asc ");
