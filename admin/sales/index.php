        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT *, COALESCE((SELECT COUNT(product_id) FROM `sales_transaction_items` where sales_transaction_id = sales_transaction.id), 0) as `items` from `sales_transaction` order by unix_timestamp(date_created) desc ");
					...
					?>
						<tr>
							...
							<td class="text-right"><?php echo format_num($row['items']) ?></td>
